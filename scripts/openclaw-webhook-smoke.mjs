#!/usr/bin/env node
/**
 * OpenClaw webhook smoke test.
 * Sends a harmless ping payload to OPENCLAW_WEBHOOK_URL if configured.
 * Fails clearly when env vars are missing — never calls production without explicit config.
 * Never performs destructive actions.
 */

const url = process.env.OPENCLAW_WEBHOOK_URL?.trim();
...
if (!url) {
  console.log(`\n${YELLOW}⚠  SKIP${NC} — OPENCLAW_WEBHOOK_URL is not set.`);
  console.log(`   Set OPENCLAW_WEBHOOK_URL (and optionally OPENCLAW_WEBHOOK_TOKEN) to run this smoke test.`);
...
try {
  ...
  if (res.ok || res.status === 404 || res.status === 405) {
    // 404/405 means endpoint exists but doesn't accept pings yet — that's fine for a smoke test
    console.log(`\n${GREEN}✔  OpenClaw webhook reachable${NC}\n`);
    process.exit(0);
  } else {
