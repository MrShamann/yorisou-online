...
function tableSql(table) {
  ...
  //
  // That statement is silent when the name is already taken by something of a DIFFERENT shape: it
  // reports success, the migration ledger goes green, and every function that reads the table fails
  // at runtime on a missing column. It is the exact failure 202607300002 was written to avoid —
  // Production holds `yorisou_recommendation_{sets,items,actions}` from the legacy recommendation
  // graph, with real rows, so the CPC-1 family was renamed into the canonical namespace rather than
  // promoted on top of them.
  //
  // The canonical names are therefore genuinely absent from Production and this DOES create. The
  // guard is for the case nobody plans: if the name is somehow already taken, the release stops
  // here rather than at the first user request.
  out.push(
    [
      `do $$`,
      `declare`,
      `  v_unexpected text;`,
      `begin`,
      `  if to_regclass('${q(table.name)}') is null then`,
      ...
      `       where a.attrelid = '${q(table.name)}'::regclass and a.attnum > 0 and not a.attisdropped`,
      `         and a.attname <> all (array[${columnNames}])`,
      `      union all`,
      `      select c.name from unnest(array[${columnNames}]) c(name)`,
      `       where not exists (`,
      `         select 1 from pg_attribute a`,
      `          where a.attrelid = '${q(table.name)}'::regclass and a.attname = c.name`,
      `            and a.attnum > 0 and not a.attisdropped`,
      `       )`,
      `    ) s;`,
      `    if v_unexpected is not null then`,
      `      raise exception 'POR-1: ${table.name} already exists with a different shape (differing column(s): %). Refusing to promote onto it.', v_unexpected;`,
      `    end if;`,
      `  end if;`,
      `end $$;`,
    ].join("\n"),
