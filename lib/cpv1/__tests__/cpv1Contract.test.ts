// CPV1-CM0 — CLEAN-MAIN foundation contract (pure lib/cpv1 only; NO branch-only app
// imports). Ported from the stacked-branch cpv1Completion.test.ts with every
// APP-2/AIX/product-card/LINE-dependent check removed (split per CM0 §5). A lower but
// TRUTHFUL clean-main test count — no fake dependency retention.
...
console.log("CPV1-CM0 §5 — runtime-truth reconciliation (registry-only; route markup is app-layer, not CPV1)");
// CM0: the route-file-existence + flow-mount checks read branch/app markup and are
// NOT part of the CPV1 lib foundation — route-on-`main` presence is git-verified in
// CM0 evidence (CM0_DEPENDENCY_MAP.md), not in this pure contract test. Here we verify
// only the registry truth: exactly 9 route-verified methods, and the exact identities.
const CM0_ROUTE_VERIFIED_IDS = [
  "imairo-120q", "c02-current-state", "relationship-fatigue-24q", "f01-work-fit",
  "f02-workplace-fit", "love-distance", "work-rhythm", "local-life", "name-impression",
];
check("§5 exactly 9 production-route-verified methods (registry truth); publicMethods()===0", () => {
  const rv = productionRouteVerifiedMethods().map((m) => m.methodId).sort();
  assert.deepEqual(rv, [...CM0_ROUTE_VERIFIED_IDS].sort(), "the 9 route-verified methods, by id");
  assert.equal(publicMethods().length, 0, "no method is Founder-activated ⇒ 0 public_active");
});
...
console.log("CPV1-CM0 — clean-main foundation files present (self-contained)");
check("all 7 CPV1 contract modules present", () => {
