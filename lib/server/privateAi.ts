...
async function callProvider(prompt: string, enabled: { mistral_enabled: boolean; openrouter_enabled: boolean }) {
  ...
  for (const attempt of attempts) { try { const started = Date.now(); const response = await attempt.call(); if (!response.ok) { lastError = `provider_http_${response.status}`; continue; } const data = await response.json() as { choices?: Array<{ message?: { content?: string } }>; usage?: { prompt_tokens?: number; completion_tokens?: number } }; const raw = data.choices?.[0]?.message?.content; if (!raw) { lastError = "provider_empty"; continue; } return { content: validateReflection(JSON.parse(raw)), provider: attempt.provider, model: attempt.model, latency: Date.now() - started, inputTokens: data.usage?.prompt_tokens || null, outputTokens: data.usage?.completion_tokens || null }; } catch (error) { lastError = error instanceof Error && error.name === "TimeoutError" ? "provider_timeout" : "provider_error"; } }
