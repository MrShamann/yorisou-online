...
async function callProvider(prompt: string, enabled: { mistral_enabled: boolean; openrouter_enabled: boolean }) {
  const sharedRoutes = resolvePrivateReflectionProviders();
  const attempts: Array<{ provider: string; model: string; endpoint: string; headers: Record<string, string>; body: (value: string) => Record<string, unknown>; parse: (value: unknown) => { content: string | null; inputTokens: number | null; outputTokens: number | null } }> = sharedRoutes.length > 0
    ? sharedRoutes
    : [];
  if (attempts.length === 0 && enabled.mistral_enabled && process.env.MISTRAL_API_KEY?.trim()) attempts.push({ provider: "mistral", model: process.env.MISTRAL_MODEL || "mistral-small-latest", endpoint: "https://api.mistral.ai/v1/chat/completions", headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.MISTRAL_API_KEY}` }, body: (value) => ({ model: process.env.MISTRAL_MODEL || "mistral-small-latest", temperature: 0.25, max_tokens: 1100, response_format: { type: "json_object" }, messages: [{ role: "system", content: "Return only valid JSON." }, { role: "user", content: value }] }), parse: (value) => { const data = value as { choices?: Array<{ message?: { content?: string } }>; usage?: { prompt_tokens?: number; completion_tokens?: number } }; return { content: data.choices?.[0]?.message?.content || null, inputTokens: data.usage?.prompt_tokens || null, outputTokens: data.usage?.completion_tokens || null }; } });
  if (attempts.length === 0 && enabled.openrouter_enabled && process.env.OPENROUTER_API_KEY?.trim()) attempts.push({ provider: "openrouter", model: process.env.YORISOU_PRIVATE_AI_OPENROUTER_MODEL || "mistralai/mistral-small-3.1-24b-instruct", endpoint: `${process.env.OPENROUTER_BASE_URL?.replace(/\/$/, "") || "https://openrouter.ai/api/v1"}/chat/completions`, headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}` }, body: (value) => ({ model: process.env.YORISOU_PRIVATE_AI_OPENROUTER_MODEL || "mistralai/mistral-small-3.1-24b-instruct", temperature: 0.25, max_tokens: 1100, response_format: { type: "json_object" }, messages: [{ role: "system", content: "Return only valid JSON." }, { role: "user", content: value }] }), parse: (value) => { const data = value as { choices?: Array<{ message?: { content?: string } }>; usage?: { prompt_tokens?: number; completion_tokens?: number } }; return { content: data.choices?.[0]?.message?.content || null, inputTokens: data.usage?.prompt_tokens || null, outputTokens: data.usage?.completion_tokens || null }; } });
  let lastError = "provider_unavailable";
  for (const attempt of attempts) { try { const started = Date.now(); const response = await fetch(attempt.endpoint, { method: "POST", signal: AbortSignal.timeout(20000), headers: attempt.headers, body: JSON.stringify(attempt.body(prompt)) }); if (!response.ok) { const body = await response.json().catch(() => null) as { error?: { code?: unknown } } | null; const code = typeof body?.error?.code === "string" && /^[a-z0-9_-]{1,48}$/i.test(body.error.code) ? `_${body.error.code}` : ""; lastError = `provider_http_${response.status}${code}`; continue; } const parsed = attempt.parse(await response.json()); if (!parsed.content) { lastError = "provider_empty"; continue; } return { content: validateReflection(JSON.parse(parsed.content)), provider: attempt.provider, model: attempt.model, latency: Date.now() - started, inputTokens: parsed.inputTokens, outputTokens: parsed.outputTokens }; } catch (error) { lastError = error instanceof Error && error.name === "TimeoutError" ? "provider_timeout" : "provider_error"; } }
