...
export async function deleteAssessmentResult(resultRowId: string, ownerAccountId: string): Promise<boolean> {
  return rpc<boolean>("yorisou_assessment_result_delete", {
...
export function deriveCurrentUnderstanding(result: AssessmentResult, responses: InterpretationResponse[]) {
  ...
  const acceptedResultId =
    latest?.response_type === "corrected" && latest.corrected_result_id
      ? latest.corrected_result_id
      : latest?.response_type === "rejected"
        ? null
        : result.result_id;
  return {
    originalResultId: result.original_result_id,
    acceptedResultId,
    status: latest?.response_type ?? "unanswered",
    respondedAt: latest?.created_at ?? null,
    recommendationUsePermitted: latest ? latest.recommendation_use_permitted : true,
    continuityUsePermitted: latest ? latest.continuity_use_permitted : true,
