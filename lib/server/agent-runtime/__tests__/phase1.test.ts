...
export async function runAgentRuntimePhase1Tests() {
  ...
  await fixture(async (r,m) => { const f="README.md"; const v=(await readFile(path.join(r,f),"utf8")).replace("- `Yorisou_AI_Native_Product_and_Agent_Capability_Map_v0.3.1.md`","- `Yorisou_AI_Native_Product_and_Agent_Capability_Map_v0.3.1.md`\n- `Yorisou_AI_Native_Product_and_Agent_Capability_Map_v0.3.1.md`"); await writeFile(path.join(r,f),v); m[f]=digest(v); }, /manifest_entries/);
  await fixture(async (r,m) => { const f="README.md"; const v=(await readFile(path.join(r,f),"utf8")).replace("Yorisou_AI_Native_Product_and_Agent_Capability_Map_v0.3.1.md","../escape.md"); await writeFile(path.join(r,f),v); m[f]=digest(v); }, /allowlist_mismatch/);
  await fixture(async (r,m) => { const f="Yorisou_Project_Constitution_v0.3.1.md"; const v=(await readFile(path.join(r,f),"utf8")).replace("remain **Planned**", "are **Current**"); await writeFile(path.join(r,f),v); m[f]=digest(v); }, /required_governance_assertion/);
  await fixture(async (r,m) => { const f="Yorisou_Digital_Legacy_and_Synthetic_Identity_Governance_v0.1.md"; const v=(await readFile(path.join(r,f),"utf8")).replace("**Implementation Status:** Not active", "**Implementation Status:** Active"); await writeFile(path.join(r,f),v); m[f]=digest(v); }, /required_governance_assertion/);
