#!/usr/bin/env bash
# DCI-1.1 §9 — authenticated FULL-STACK local acceptance (single documented command):
...
# browser acceptance (tests/smoke/daily-check-in-fullstack.spec.ts) through the
...
APP_PORT=3200
WORK="${DCI_FULLSTACK_WORK:-/tmp/dci11-fullstack}"
PGDIR="$WORK/pg"
DATADIR="$WORK/auth-store"
PGBIN=/opt/homebrew/opt/postgresql@16/bin
DB=yorisou_daily_check_in_test
REST_CONTAINER=dci11-postgrest

cleanup() {
  set +e
  if [[ "${DCI_FULLSTACK_KEEP:-}" == "1" ]]; then echo "[fullstack] KEEP=1 — stack left running for debugging"; return; fi
...
JWT_SECRET="dci11-local-fullstack-secret-$(date +%s)-0123456789abcdef"
SERVICE_KEY=$(S="$JWT_SECRET" node -e "const c=require('crypto');const b=(o)=>Buffer.from(JSON.stringify(o)).toString('base64url');const h=b({alg:'HS256',typ:'JWT'});const p=b({role:'service_role',iss:'dci11-local',exp:Math.floor(Date.now()/1000)+86400});const s=c.createHmac('sha256',process.env.S).update(h+'.'+p).digest('base64url');console.log(h+'.'+p+'.'+s)")
...
for i in $(seq 1 60); do
  REST_CHECK=$(curl -s -o /dev/null -w '%{http_code}' -H "apikey: $SERVICE_KEY" -H "Authorization: Bearer $SERVICE_KEY" "http://localhost:$REST_PORT/yorisou_daily_state_records?select=id&limit=1" || true)
...
YORISOU_AUTH_COOKIE_SECRET="dci11-local-cookie-secret-0123456789abcdef0123456789abcdef" \
...
DCI_FULLSTACK=1 \
DCI_REST_URL="http://localhost:$REST_PORT" \
DCI_SERVICE_KEY="$SERVICE_KEY" \
PLAYWRIGHT_BASE_URL="http://localhost:$APP_PORT" \
npx playwright test tests/smoke/daily-check-in-fullstack.spec.ts --project=desktop
