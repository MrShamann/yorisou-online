...
| 10 | **Original computation, recording, interpretation or import model** (MTF-1.3 — replaces the scoring-only step; execution-model aware). The required artifact depends on the method's `MethodExecutionModel`: **scored** → weights/rules, scoring version, tie-break, fallback, result assembly, confidence policy; **recorded_state** → state schema, allowed values, timestamp/cadence rules, validation rules, NO score, `yorisouScoring: null`; **symbolic** → source/school, input-to-symbol or calculation rules, interpretation version, deterministic status only where truthful, boundary language, no scientific weighting; **imported_external** → provider mapping, import verification mode, label handling, ownership boundary, NO YORISOU scoring; **entertainment** → draw/selection mechanism, output pool, share-safe copy behavior, exclusion policy, no psychology scoring requirement. A recorded **`not_applicable` scoring decision is explicitly ALLOWED** for non-scored execution models — but "not applicable" never bypasses the required execution-model artifact above. | execution-model artifact (variant-appropriate, versioned) |
