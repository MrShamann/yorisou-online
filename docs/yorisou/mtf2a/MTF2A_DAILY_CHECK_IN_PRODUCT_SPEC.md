...
**Name:** 「きょうの空模様」／subtitle 「30秒のじぶん記録」. Collision-checked: distinct from 今のわたしチェック (C02), 今日のおみくじ (S01), and every existing result/persona/companion name.
...
## 2. State schema (`daily-state-schema-v1`)
...
- Timestamp: `producedAt` = device-local save time; one entry per calendar day; same-day re-entry overwrites after confirmation.
...
## 4. Acknowledgement model (`daily-ack-v1`)
...
Tags (`rest_hint`, `organize_hint`, `connect_hint`, `solo_time_hint`, `small_win_hint`) derive 1:1 from `kyou_hoshii` — optional hints only, no verdicts, **no commercial products in public copy, no payment/pricing/urgency anywhere** (none is implemented).
