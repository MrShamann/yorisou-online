...
| 6 | Authenticated a11y not in CI | `CLOSED` | **The stated blocker was not real.** PostgREST has run in this repository's CI since `dci-1-ci.yml` — v12.2.3, pinned, in Docker. The blocker was Docker on the ACCEPTANCE MACHINE, never a property of CI. `osf1-life-ci.yml` runs the same harness against PostgreSQL 17 and the pinned image, and is **GREEN**: axe 32, keyboard 12, audit-failure E2E 2, both reflection modes 3. No new supply-chain decision was needed. Getting there took three real harness defects out — a `lsof`-based port pre-flight that read a missing command as "the port is free", a teardown that killed `npx` and left the server holding the port, and steps sharing ports — each of which had been invisible while the harness only ever ran once at a time on one machine |
