...
| 16 | Authenticated a11y in CI | `test:osf1-a11y-authenticated` runs locally; **now in `osf1-life-ci.yml`, green** | **GAP** | Wire into CI on disposable PG + PostgREST + production build, — DONE: the recorded blocker was never true of CI, and the job drives the same harness | A green scan against a sign-in wall is worse than no scan | CI job |
