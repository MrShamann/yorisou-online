...
export async function generateStaticParams() {
  return (await fetchNews("ja")).map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata({
  ...
  const insight = await fetchInsightDetail(slug, "ja");
      ...
      <article className="mx-auto max-w-5xl px-6 py-14 md:px-10 md:py-18">
        ...
        <section className="mt-8 rounded-[1.75rem] border border-[#D6C3A3]/35 bg-white/80 p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <AudienceCard title="地域・コミュニティへ" text={insight.whatThisMeans.localCommunities} />
            <AudienceCard title="運営者・実証パートナーへ" text={insight.whatThisMeans.operators} />
          </div>
        </section>
