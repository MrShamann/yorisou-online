...
export default async function InsightsPreview({ locale }: { locale: Locale }) {
  let hero: Awaited<ReturnType<typeof fetchHomepageInsights>>["hero"] = [];
  let secondary: Awaited<ReturnType<typeof fetchHomepageInsights>>["secondary"] = [];

  try {
    const fetched = await fetchHomepageInsights(locale, 1, 1);
    hero = fetched.hero;
    secondary = fetched.secondary;
  } catch (error) {
