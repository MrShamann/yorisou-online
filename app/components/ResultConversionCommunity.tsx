...
export default function ResultConversionCommunity({ moduleId, reportTeaser }: Props) {
  const data = CONVERSION_DATA[moduleId];
  const { communityVoices, reportPreview, nextChecks, lineSaveCopy } = data;

  const [nearCount, setNearCount] = useState(0);
  const [nearClicked, setNearClicked] = useState(false);

  function handleNearClick() {
    if (!nearClicked) {
      setNearCount((n) => n + 1);
      setNearClicked(true);
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Community voices ── */}
      <div className="rounded-[1.35rem] border border-[rgba(23,59,53,0.1)] bg-white/80 p-5 space-y-4">
        <div className="space-y-1">
          <p className="text-[13px] font-semibold tracking-[0.1em] text-[#49615B]">同じような人の声</p>
          <p className="text-[12px] leading-6 text-[#7A7068]">
            今の状態に近い声を見るだけでも大丈夫です。ここは公開投稿の場所ではありません。
          </p>
        </div>
        ...
        <div className="space-y-2.5">
          {communityVoices.map((voice, i) => (
            <div
              key={i}
              className="rounded-[1rem] border border-[rgba(23,59,53,0.09)] bg-[#F3FAF6] px-4 py-3"
            >
              <p className="text-[14px] leading-7 text-[#2F2A28]">「{voice.text}」</p>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-[1.35rem] border border-[rgba(23,59,53,0.1)] bg-white/80 p-5 space-y-3">
        <div className="space-y-1">
          <p className="text-[13px] font-semibold text-[#49615B]">{reportPreview.noFearCta}</p>
          <p className="text-[14px] font-semibold leading-6 text-[#2F2A28]">{reportPreview.headline}</p>
        </div>
        <p className="text-[13px] leading-7 text-[#5F5750]">{reportPreview.teaserText}</p>
        {reportTeaser && (
          <p className="text-[13px] leading-6 text-[#5F5750] italic">{reportTeaser}</p>
        )}
        ...
        <span className="inline-flex rounded-full border border-[rgba(23,59,53,0.18)] bg-[#F8F7F4] px-4 py-2 text-[13px] font-semibold text-[#7A7068]">
          プレビューを見る（準備中）
        </span>
      </div>
      ...
      <div className="rounded-[1.25rem] border border-[rgba(217,130,86,0.14)] bg-[#FFF7EC]/70 px-5 py-4 space-y-1.5">
        <p className="text-[13px] font-semibold text-[#6B4E3F]">
          LINEで{lineSaveCopy}（準備中）
        </p>
        <p className="text-[12px] leading-6 text-[#7A7068]">
      ...
      {nextChecks.length > 0 && (
        <div className="space-y-2">
          <p className="text-[12px] font-semibold tracking-[0.08em] text-[#49615B]">次のチェックを見る</p>
      ...
      {/* ── Soft exit ── */}
      <p className="text-[12px] leading-6 text-[#9A918B] text-center">
