    ...
    <main className="min-h-screen bg-[var(--yorisou-color-surface-bg)] text-[var(--yorisou-color-neutral-800)]">
      <div className="container py-8 md:py-14">
        <div className="mx-auto max-w-[42rem] space-y-5">
          ...
          {/* ── Section 1: Result identity with state object ── */}
          <div className="yorisou-reveal yorisou-glow-field space-y-4 rounded-[var(--yorisou-radius-hero)] border border-[var(--yorisou-color-neutral-100)] bg-[var(--yorisou-color-surface-card)] p-6 shadow-[var(--yorisou-shadow-card)] md:p-7">
            ...
            <div className="flex items-center gap-5">
              <div className="yorisou-orb yorisou-orb-float flex h-[74px] w-[74px] shrink-0 items-center justify-center md:h-[92px] md:w-[92px]" aria-hidden="true">
                <YorisouSymbol variant="white" size={36} breathing />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] font-bold tracking-[0.1em] text-[var(--yorisou-color-primary-600)]">あなたの結果 · 人間関係の疲れ</p>
                <h1 className="mt-1 text-[1.48rem] font-bold leading-[1.32] text-[var(--yorisou-color-neutral-800)] md:text-[1.95rem]">
                  {archetype.name}
                </h1>
              </div>
            </div>
            <p className="text-[14.5px] leading-8 text-[var(--yorisou-color-neutral-500)]">{archetype.body}</p>
            <p className="text-[12px] leading-6 text-[var(--yorisou-color-neutral-500)] opacity-90">
              今の状態の整理であり、あなたを決めつけるものではありません。
            </p>
          </div>

          {/* ── Section 2: AI interpretation (deep surface) ── */}
          <div className="yorisou-reveal-late rounded-[var(--yorisou-radius-card)] bg-[var(--yorisou-color-deep-900)] p-5 text-white md:p-6">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="rounded-[var(--yorisou-radius-pill)] bg-white/10 px-2.5 py-0.5 text-[10.5px] font-semibold text-white/60">今の状態</span>
            </div>
            <p className="mt-3 text-[14px] leading-8 text-white/92">{archetype.insight}</p>
            <div className="mt-5 border-t border-white/10 pt-4">
              <p className="text-[11px] font-bold tracking-[0.1em] text-white/60">3つの気づき</p>
              <div className="mt-2.5 space-y-2.5">
                {[
                  { n: "01", label: "疲れが出やすい場面", value: archetype.fatigueScene },
                  { n: "02", label: "守りたい距離", value: archetype.boundaryHint },
                  { n: "03", label: "戻りやすい形", value: archetype.recoveryHint },
                ].map(({ n, label, value }) => (
                  <div key={n} className="flex gap-3">
                    <span className="mt-0.5 text-[12px] font-bold text-[var(--yorisou-color-accent-500)]">{n}</span>
                    <div className="min-w-0">
                      <p className="text-[11.5px] font-bold text-white/60">{label}</p>
                      <p className="mt-0.5 text-[13.5px] leading-7 text-white/90">{value}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* ── Section 3: 今日の小さな一歩 (action card) ── */}
          <div className="rounded-[var(--yorisou-radius-card)] border border-[var(--yorisou-color-primary-100)] bg-white p-5 shadow-[var(--yorisou-shadow-card)]">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11.5px] font-semibold text-[var(--yorisou-color-neutral-500)]">短い時間でOK · 無理のない範囲で</span>
            </div>
            <p className="mt-3 text-[15.5px] font-bold leading-8 text-[var(--yorisou-color-neutral-800)]">{archetype.nextStep}</p>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <span className="text-[12.5px] text-[var(--yorisou-color-neutral-500)]">今日はここまでにしても大丈夫です。</span>
            </div>
          </div>
