...
export function RelationshipFatiguePrivateSave({
  ...
  return (
    <div className="space-y-5">
      {/* Private save */}
      <section className="rounded-[1.25rem] border border-[var(--yr-hair)] bg-[var(--yr-panel)] px-5 py-5">
        <p className="text-[13px] font-semibold text-[color:var(--yr-accent-text)]">この結果を非公開で残す</p>
        ...
        {state === "saved" && savedId ? (
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex min-h-[46px] items-center justify-center rounded-full border border-[var(--yr-accent)] bg-[var(--yr-accent)] px-5 text-[14px] font-semibold text-[color:var(--yr-accent-ink)]"
            >
              保存した結果を見る
            </Link>
          </div>
          ...
          <div className="mt-4 space-y-2.5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                href={lineHref}
                className="inline-flex min-h-[46px] items-center justify-center rounded-full border border-[var(--yr-hair-2)] bg-[var(--yr-panel)] px-5 text-[14px] font-semibold text-[color:var(--yr-accent-text)]"
              >
                LINEでログイン
              </Link>
            </div>
          ...
          <button
            ...
            className="mt-4 inline-flex min-h-[46px] items-center justify-center rounded-full border border-[var(--yr-accent)] bg-[var(--yr-accent)] px-5 text-[14px] font-semibold text-[color:var(--yr-accent-ink)] disabled:opacity-60"
          >
      ...
      {state === "saved" ? (
        <section className="rounded-[1.25rem] border border-[var(--yr-hair)] bg-[var(--yr-panel-2)] px-5 py-5">
          <p className="text-[13px] font-semibold text-[color:var(--yr-accent-text)]">今の状態に近い、小さな選択肢</p>
          {recState === "loading" ? (
            <p className="mt-2 text-[13px] leading-7 text-[color:var(--yr-text-mut)]">候補を整理しています。</p>
          ) : recState === "loaded" && topItem ? (
            <div className="mt-3 space-y-3">
              <div className="rounded-[1rem] border border-[var(--yr-hair)] bg-[var(--yr-panel)] px-4 py-4">
                <p className="text-[11px] font-semibold tracking-[0.1em] text-[color:var(--yr-kicker)]">
                  {topItem.object_type === "experience_card"
                    ? "匿名で共有された体験"
                    : topItem.yorisou_resources?.resource_type || "選択肢"}
                </p>
                <p className="mt-1.5 text-[15px] font-semibold leading-7 text-[color:var(--yr-text)]">
                ...
                <p className="mt-2.5 text-[12px] leading-6 text-[color:var(--yr-text-faint)]">{topItem.reason}</p>
                <p className="mt-1 text-[11px] leading-6 text-[color:var(--yr-text-faint)]">{topItem.disclosure}</p>
              </div>
              {feedbackDone ? (
                <p className="text-[13px] leading-7 text-[color:var(--yr-accent-text)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {FEEDBACK_ACTIONS.map(([action, label]) => (
                    <button
                      ...
                      className="inline-flex min-h-[40px] items-center justify-center rounded-full border border-[var(--yr-hair-2)] bg-[var(--yr-panel)] px-4 text-[13px] font-semibold text-[color:var(--yr-accent-text)] transition hover:-translate-y-0.5"
                    >
                      {label}
                    </button>
                  ))}
            ...
            <p className="mt-2 text-[13px] leading-7 text-[color:var(--yr-text-mut)]">
              今は表示できる候補がありませんでした。保存した結果からいつでも見直せます。
            </p>
          )}

          {/* Return reason */}
          <div className="mt-4 border-t border-[var(--yr-hair)] pt-4">
            <p className="text-[12px] font-semibold tracking-[0.1em] text-[color:var(--yr-kicker)]">このあとの入口</p>
            <div className="mt-2.5 flex flex-wrap gap-2">
              {recommendation && !returnPlanned ? (
                <button
                  type="button"
                  onClick={() => planReturn(recommendation.set.id)}
                  className="inline-flex min-h-[42px] items-center justify-center rounded-full border border-[var(--yr-hair-2)] bg-[var(--yr-panel)] px-4 text-[13px] font-semibold text-[color:var(--yr-accent-text)]"
                >
                  1週間後の変化を記録する
                </button>
              ) : null}
            ...
            {returnPlanned ? (
              <p className="mt-2.5 text-[12px] leading-6 text-[color:var(--yr-text-faint)]">
