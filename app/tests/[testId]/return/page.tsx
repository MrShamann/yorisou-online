...
export default function TestSaveReturnPage({ params }: { params: Promise<{ testId: string }> }) {
  ...
  return <main className="min-h-[calc(100dvh-3.5rem)] bg-[#FBFAF6] text-[#2F2A28]"><div className="container py-12"><div className="mx-auto max-w-lg rounded-[1.25rem] bg-white p-6">{savedId ? <><p>結果を非公開で保存しました。</p><Link href={`/saved/tests/${savedId}`} className="btn mt-5 inline-flex rounded-full px-5 py-3">保存した結果を開く</Link></> : state === "saving" ? <p>結果を非公開で保存しています。</p> : <><p>{state === "missing" ? "保存する結果が見つかりませんでした。" : "結果を保存できませんでした。"}</p><Link href={`/tests/${testId || "c02"}`} className="btn mt-5 inline-flex rounded-full px-5 py-3">テストに戻る</Link></>}</div></div></main>;
}
