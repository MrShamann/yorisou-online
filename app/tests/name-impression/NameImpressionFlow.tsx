          ...
          <div className="mx-auto max-w-[42rem] space-y-6">
            ...
            <ShareResultActions
              input={{
                ...
                seed: result.id,
                url: "/tests/name-impression",
              }}
              trackingTestId="name-impression"
            />
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <h1 className="display-serif text-[2rem] leading-[1.22] text-[color:var(--yr-text)] md:text-[2.6rem]">
                  名前から受ける印象を、<br />
                  <span className="text-[color:var(--yr-accent-text)]">今の見え方のヒントにする。</span>
                </h1>
              ...
              <div className="rounded-[1.35rem] border border-[var(--yr-hair)] bg-[var(--yr-panel)] p-5 shadow-[0_16px_36px_rgba(23,59,53,0.07)] space-y-3">
                <label className="block space-y-2">
                  <span className="text-[13px] font-semibold text-[color:var(--yr-accent-text)]">呼ばれたい名前やニックネーム</span>
                  <input
                    value={displayName}
                    onChange={(event) => setDisplayName(event.target.value)}
                    placeholder="例: あや / Aya"
                    className="w-full rounded-[1rem] border border-[var(--yr-hair-2)] bg-[var(--yr-panel)] px-4 py-3 text-[15px] text-[color:var(--yr-text)] outline-none transition focus:border-[var(--yr-accent)]"
                  />
                </label>
                <p className="text-[12px] leading-6 text-[color:var(--yr-text-faint)]">入力した名前はこの端末上だけで使われ、送信されません。</p>
              </div>
              <button
                type="button"
                onClick={begin}
                disabled={introDisabled}
                className="inline-flex min-h-[56px] w-full items-center justify-center rounded-full border border-[var(--yr-accent)] bg-[var(--yr-accent)] px-6 text-[16px] font-bold text-[color:var(--yr-accent-ink)] shadow-[0_18px_34px_rgba(23,59,53,0.22)] transition hover:-translate-y-0.5 hover:bg-[#0F2F2B] disabled:cursor-not-allowed disabled:bg-[rgba(111,98,92,0.26)] sm:w-auto"
              >
                印象を見てみる
              </button>
            </div>
