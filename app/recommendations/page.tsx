...
const primaryWhyByResult: Record<string, string> = {
  open_stride: "今の結果では、流れを広げやすいので、正式版の案内を先に見ると次の見通しがつかみやすいです。",
  balanced_window: "今の結果では、無理のない整え方が合いやすいので、正式版の案内を静かに受け取るのに向いています。",
  sorting_entry: "今の結果では、整理の入口が手がかりになります。正式版の見え方が合いやすいです。",
  switching_wave: "今の結果では、場面ごとの切り替えが手がかりです。正式版の案内で見方をそろえると読みやすいです。",
  quiet_restore: "今の結果では、休み方と戻り方が先に効きやすいです。まず正式版の案内でそこを見ておくと合います。",
};
...
export default async function RecommendationsPage({
  ...
  const primaryRecommendation = {
    kind: "正式版の案内",
    title: "正式版の案内を見る",
    whyFit: primaryWhyByResult[result.id] ?? primaryWhyByResult[currentStateCheckV1.scoring.fallbackResultId],
    href: "/formal-check",
    cta: "正式版の案内を見る",
  };
  const secondaryRecommendation = {
    kind: "もう一度チェック",
    title: "もう一度クイックチェックする",
    whyFit: "別の日に見比べると、今の傾きが読みやすくなります。",
    href: "/check-in",
    cta: "もう一度クイックチェックする",
  };
        ...
        <div className="container grid gap-5 py-5 md:py-10 lg:grid-cols-[0.96fr_1.04fr] lg:items-start lg:gap-8">
          <div className="max-w-[40rem] space-y-4">
            ...
            <div className="space-y-3">
              <p className="service-kicker">{result.displayName}の次に見るもの</p>
              ...
              <p className="max-w-[34rem] text-[15px] font-medium leading-7 text-[#6F625C]">
                たくさん並べず、今の結果のあとで役立ちそうな方向だけをやさしく置いています。
              </p>
            </div>
            <div className="grid gap-3">
              <MvpCard className="space-y-3 rounded-[1.25rem] border-[rgba(23,59,53,0.12)] bg-[radial-gradient(circle_at_16%_0%,_rgba(217,164,65,0.2),_transparent_46%),linear-gradient(180deg,_#fffefd,_#F4FAF7)] shadow-[0_22px_44px_rgba(23,59,53,0.1)]">
                <div className="flex items-center justify-between gap-3">
                  <div className="service-kicker">{primaryRecommendation.kind}</div>
                  <MvpPill>おすすめ</MvpPill>
                </div>
                <h2 className="display-serif text-[1.55rem] leading-[1.25] text-[#2F2A28]">{primaryRecommendation.title}</h2>
                <p className="text-[14px] font-medium leading-7 text-[#4A3E39]">{primaryRecommendation.whyFit}</p>
                <MvpActionLink
                  href={primaryRecommendation.href}
                  label={primaryRecommendation.cta}
              ...
              <MvpCard className="space-y-3 rounded-[1.15rem] border-[rgba(105,151,130,0.14)] bg-white/92 shadow-[0_14px_30px_rgba(105,151,130,0.08)]">
                <div className="flex items-center justify-between gap-3">
                  <div className="service-kicker">{secondaryRecommendation.kind}</div>
                  <MvpPill>次の候補</MvpPill>
                </div>
                <h2 className="display-serif text-[1.45rem] leading-[1.38]">{secondaryRecommendation.title}</h2>
                <p className="text-[14px] leading-7 text-[var(--text)]">{secondaryRecommendation.whyFit}</p>
                <MvpActionLink
                  href={secondaryRecommendation.href}
                  label={secondaryRecommendation.cta}
                  tone="secondary"
