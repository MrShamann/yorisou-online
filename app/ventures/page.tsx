...
import VenturesView from "@/app/_corporate/p5r2/views/VenturesView";
...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  return localeMetadata(searchParams, "ventures");
}
...
export default async function Page({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <Shell copy={copy} locale={locale} path="/ventures">
      <VenturesView copy={copy} locale={locale} />
    </Shell>
  );
}
